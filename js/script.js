    ...
    function renderEquipas(data, container) {
        ...
        data.forEach(equipa => {
            ...
            if (equipa.instagram_url) {
                const username = getInstagramUsername(equipa.instagram_url);
                // Use instagram:// scheme for mobile app, fallback to web URL
                const instagramAppUrl = `instagram://user?username=${username}`;
                const instagramWebUrl = `https://www.instagram.com/${username}/`;

                socialIcons += `<a href="${instagramWebUrl}" 
                    onclick="event.preventDefault(); 
                             window.location.href='${instagramAppUrl}'; 
                             setTimeout(function(){ window.open('${instagramWebUrl}', '_blank'); }, 500);" 
