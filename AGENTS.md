...
- [2026-09-08] Gemini: Criação da opção 'Plantel' nos menus do Treinador e Diretor com listagem de atletas e Ficha Individual (foto, dados, KPIs de assiduidade em tempo real e timeline de registos com observações do Diário Desportivo), mantendo a aba Contactos SOS dedicada.
- [2026-09-08] Gemini: Harmonização visual da listagem do Plantel para seguir exatamente o layout e padrão dos cartões do Diário Desportivo (.atleta-presenca-card com .atleta-info-row, avatar 44px e botão de ação .btn-plantel-ficha).
...
- [2026-09-21] Gemini: Sistema de Links Diretos e Partilha Social de Notícias: criação de noticia.html dedicada, abertura automática por parâmetro (?noticia=ID) na Home com barra de partilha (WhatsApp/Facebook/Copiar Link) e botões de ação rápida no Portal do Redator e Admin.
