...
- [2026-08-27] Gemini: Correções no formulário de Atletas do Admin (Sexo, Época, número de camisola, dados de Encarregado e resolução segura de atletas por ID nos botões de ação).
...
- [2026-09-25] Gemini: Implementação do Sistema de Sincronização Oficial da FPB (Federação Portuguesa de Basquetebol, ID Clube: 656) para Agenda e Resultados: Edge Function sync-fpb, automação cron no Supabase (setup_cron_sync_fpb.sql), botão 'Sincronizar com FPB' no Admin com modal interativo e pré-visualização, registo manual de resultados e ação 'Finalizar Jogo' com 1 toque na Agenda.
- [2026-09-25] Gemini: Criação da secção oficial Competições (#competicoes) no site principal (index.html, style.css, main.js) destacando as provas do clube na Época 2026/2027 (CN2 Seniores, Sub 18, Sub 16, Sub 14, Minibasquete) com links para plantéis e correspondência dinâmica com equipasbcv.
